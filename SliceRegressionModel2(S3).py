...
"""This 2DCNN model (additional) with conventional configuration is used for residualised score prediction from adjacent three axial slices"""
...
for slice_start_no in range(slice_range_start, slice_range_end, 3):
    ...
    PthModel='/datastore/sah012/CNN-3D-images-Tensorflow/super-resolution-project/ABCD-models/models2/model_2_'+str(slice_start)+'_'+str(slice_end)+'.hdf5'
    ...
    LearningRate = 1e-4
    #Define generators
    training_generator = Functionlib.data_generator(X_train,Y_train_regress,Shape,BatchSize,Model2D)
    validation_generator = Functionlib.data_generator(X_val,Y_val_regress,Shape,BatchSize,Model2D)
    #Define the model
    model = Functionlib.get_CNN_model2(Shape,
                              Model2D,
                              filters = ([8,1],[8, 1],[16,1],[16, 1],[32,1],[32,1]),
...
df.to_csv (r'/datastore/sah012/CNN-3D-images-Tensorflow/super-resolution-project/ABCD-models/ABCDIQNet/results_train_val_slicemodel_2.csv', header=True)
